#!/usr/bin/env bash
#
# ARP gRPC compliance test suite (reference implementation).
# Drives all 5 ARP services against a live reference arp-server via grpcurl,
# using gRPC server reflection (no .proto files required).
#
...
grpc_call() {
    local method="$1"
    shift
    # The reference server exposes gRPC server reflection, so no -proto files
    # (and no googleapis import path) are required.
    "$GRPCURL" -plaintext "$@" "$ADDR" "$method" 2>&1
}
...
run_test "lifecycle-echo-agent-has-direct-url" \
    "echo-agent-001 advertises a direct_url (A2A endpoint)" required \
    test_contains "directUrl" arp.v1.AgentService/GetAgentStatus -d '{"agentId":"echo-agent-001"}'
...
run_test "lifecycle-crush-has-direct-url" \
    "crush-agent-001 advertises a direct_url (A2A endpoint)" required \
    test_contains "directUrl" arp.v1.AgentService/GetAgentStatus -d '{"agentId":"crush-agent-001"}'
